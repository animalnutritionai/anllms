...
UPDATE: the nd.get_feed_data() -> nd.calculate_feed_data() setup this
module needs is now factored out into feed_library._feed_data
(build_complete_feed_data()), shared with
feed_library.microbial_substrate (the analogous module for the
microbial-supply inputs) -- so that real pipeline call happens in one
place, not duplicated across the two modules. Behavior here is
unchanged; only the internal plumbing moved.
"""
