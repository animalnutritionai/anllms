...
optimizer itself so the engine work can be built directly against a
settled contract, rather than co-designing the spec and the engine at
the same time.
...
DMI mode (actual vs. predicted): RESOLVED this session (Sept 2026) --
see requirements_report.build_requirements_report() and
decision.evaluate_diet.evaluate_diet(), which both now accept
dmi_mode/known_dmi_kg and are wired to real logic. dmi_mode/known_dmi_kg
below are no longer placeholders; solve_diet (once built) is expected
to read them the same way -- "actual" skips DMI prediction and uses
known_dmi_kg directly for every candidate-ration model run, matching
the meaning MeasuredDMINASEM2021 (scientific/energy/dmi_measured.py)
already gives it elsewhere. Note one open question specific to the
solver, not present in the single-ration evaluate_diet case: if
dmi_mode="actual" and candidate rations diverge materially from the
ration the known_dmi_kg was measured against, a fixed measured DMI may
not reflect what the cow would actually eat on a very different diet
(see MeasuredDMINASEM2021's own known_discrepancies/limitations for the
same point). solve_diet's design should surface this, not silently
apply a stale measured value across an arbitrarily different candidate
diet -- left for solve_diet's own build session to resolve, since it
depends on how far the optimizer is allowed to roam from the current
ration.
"""
...
class SolveRequest:
    ...
    # See module docstring: DMI mode is resolved and wired elsewhere
    # (build_requirements_report / evaluate_diet); solve_diet (not yet
    # built) is expected to honor the same meaning for every candidate
    # ration it evaluates.
