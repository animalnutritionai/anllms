...
class CopperSupplyNASEM2021(KnowledgeEquation):
    """Absorbed copper supply from the diet (NASEM 2021, Eq. 20-445/20-446)."""

    name = "Absorbed copper (Cu) supply from the diet"
    citation = Citation(publication=NASEM_DAIRY_2021, chapter="6/20", equation_number="Equations 20-445, 20-446")
    variables = [
        Variable(symbol="Fd_CuInf", name="Copper intake per ingredient", unit="mg/d"),
        Variable(symbol="Fd_acCuf", name="Absorption coefficient per ingredient", unit="mg/mg"),
    ]
    formula_text = "Fd_absCuInf = Fd_CuInf * Fd_acCuf; Abs_CuIn = sum(Fd_absCuInf) across ration"
    ...
    software_reference = NASEM_DAIRY_2021_SOFTWARE
    known_discrepancies = [
        "Extracts Abs_CuIn from a full nasem_dairy model run rather than "
        "independently summing per-ingredient contributions in this codebase.",
    ]

    def calculate(self, model_output) -> EquationResult:
        value = model_output.get_value("Abs_CuIn")
        return EquationResult(value=value, unit="mg/d", inputs_used={"Source": "Abs_CuIn from shared model run"}, equation=self)
