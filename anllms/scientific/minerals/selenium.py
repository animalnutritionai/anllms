...
class SeleniumSupplyNASEM2021(KnowledgeEquation):
    """Selenium supply from the diet (NASEM 2021, part of Eq. 20-479)."""

    name = "Selenium (Se) supply from the diet"
    citation = Citation(publication=NASEM_DAIRY_2021, chapter="6/20", equation_number="Dt_SeIn, used directly in balance Equation 20-479")
    variables = [Variable(symbol="Dt_SeIn", name="Dietary selenium intake", unit="mg/d")]
    formula_text = "Supply = Dt_SeIn (no absorption coefficient applied)"
    assumptions = ["No absorption efficiency step -- raw dietary intake used directly as supply, same pattern as sulfur."]
