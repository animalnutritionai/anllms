...
WHAT THIS DELIBERATELY DOES NOT DO YET:
  - Does not support dry-cow scenarios (milk.yield_kg <= 0) -- raises
    ValueError rather than silently applying the wrong (lactating-cow)
    DMI equation. See build_requirements_report()'s guard and
    scientific/energy/dmi_lactating.py's known_discrepancies.
...
def build_requirements_report(
    ...
    if milk.yield_kg <= 0:
        raise ValueError(
            ...
            "(milk.yield_kg <= 0) yet. This codebase only implements the "
            "lactating-cow DMI equations (Eq. 2-1 / Eq. 2-2); the "
            "reference software's separate dry-cow DMI equations "
            "(Dt_DMIn_DryCow1/2) are not mapped here (see "
            "scientific/energy/dmi_lactating.py's known_discrepancies and "
            "docs/architecture.md). Running this report for a dry cow "
            "would silently apply the wrong DMI equation and every "
            "downstream requirement/supply/balance figure would inherit "
